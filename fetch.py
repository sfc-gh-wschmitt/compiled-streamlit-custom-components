...
def fetch_package(package_name, target_dir='downloaded_package'):
    ...
    subprocess.run(['pip', 'download', '--no-deps', '--dest', target_dir, package_name], check=True)
    ...
    if not downloaded_files:
        raise FileNotFoundError(f"Package {pythonic_package_name} not found in {target_dir}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch a package from PyPI and extract its web assets.")
    parser.add_argument('package_name', type=str, help="Name of the package to fetch from PyPI")
